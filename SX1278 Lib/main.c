...
int main(void)
{
  ...
  myLoRa.DIO0_port       = GPIOB;           // Dựa trên hàm MX_GPIO_Init của bạn, chân này nằm ở GPIOB
  myLoRa.DIO0_pin        = GPIO_PIN_1;
  myLoRa.hSPIx           = &hspi1;          // Bạn đang dùng SPI1


  uint16_t LoRa_status = LoRa_init(&myLoRa);
    // Tinh chỉnh cho 100ha
...
//reset lora :V
  ...
  uint8_t version_val = LoRa_read(&myLoRa, 0x42);

  char debug_msg[100];
  // Xóa sạch buffer trước khi dùng
  memset(debug_msg, 0, 100);

  if (version_val == 0x12) {
      snprintf(debug_msg, sizeof(debug_msg), "KET NOI OK! ID: 0x12\r\n");
  }
  else {
      // In gia tru 0x42
      snprintf(debug_msg, sizeof(debug_msg), "LOI 404! Doc duoc: 0x%02X (Ky vong: 0x12)\r\n", version_val);
  }
  /* USER CODE END 2 */
  ...
  while (1)
  {
    /* USER CODE END WHILE */
	  ...
	  char debug_msg[100];
	  memset(debug_msg, 0, 100); // xoa buffer

	  if (version_val == 0x12) {
	      snprintf(debug_msg, sizeof(debug_msg), "KET NOI OK! ID: 0x12\r\n");
	  }
	  else {
	      // In ra giá trị thực tế để đoán bệnh
	      snprintf(debug_msg, sizeof(debug_msg), "LOI 404! Doc duoc: 0x%02X (Ky vong: 0x12)\r\n", version_val);
	  }
//	  char send_data[200];
//
...
//
//		  HAL_UART_Transmit(&huart1, (uint8_t*)send_data, strlen(send_data), 200);
//
//		  }
//
//	  else{
...
//
//	  }
	  HAL_UART_Transmit(&huart1, (uint8_t*)debug_msg, strlen(debug_msg), 200);
	  HAL_Delay(2000);
    /* USER CODE BEGIN 3 */
  }
  /* USER CODE END 3 */
